...
-- Código original mantido - quando carregado via loadstring(game:HttpGet(...))() do GitHub,
-- o Hydrogen já fornece game:HttpGet() automaticamente
...
-- Função de fallback para notificações quando o sistema principal falha
local function FallbackNotification(title, message)
	warn("[SolixHub] " .. title .. ": " .. message)
	if StarterGui then
		pcall(function()
			StarterGui:SetCore("SendNotification", {
				Title = title or "SolixHub",
				Text = message or "Erro desconhecido",
				Duration = 5
			})
		end)
	end
...
function Task()
	...
	if not status then
		-- Usar fallback se Notification não estiver disponível
		FallbackNotification("Warning", "Key system failed to load:\n" .. tostring(res1))
		return nil, nil
	else
		return res1, res2
	end
...
if not Task then
	FallbackNotification("Error", "Failed to initialize key system. Please check your connection and try again.")
	Players.LocalPlayer:Kick("Failed to load key system. Please rejoin.")
	return
end
...
-- Carregar tela de loading (código original)
